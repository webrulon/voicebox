"""
End-to-end test for F5-TTS generation via API.

This test verifies the complete flow:
1. Create a voice profile with sample audio
2. Generate speech using F5-TTS engine
3. Verify audio file is created
4. Verify generation appears in history with engine='f5'
"""
...
def test_e2e_f5_generation(api_base_url, test_audio_file, check_server_running):
    """
    End-to-end test for F5-TTS generation.
    ...
    with open(test_audio_file, 'rb') as audio_file:
        ...
        data = {
            'name': f'E2E Test Profile F5 {time.time()}',
            'transcription': 'This is a test audio for F5-TTS generation.'
        }
    ...
    try:
        # Step 2: Generate speech with F5-TTS engine
        generation_request = {
            'profile_id': profile_id,
            'text': 'Hello, this is a test of F5-TTS engine for voice cloning.',
            'engine': 'f5',
            'model_type': 'F5TTS_v1_Base'
        }

        print(f"Generating speech with F5-TTS engine...")
        response = requests.post(
            f"{api_base_url}/generate",
            json=generation_request,
            timeout=300  # F5-TTS may take time for first run (model download)
        )
        ...
        # Step 4: Verify generation appears in history with engine='f5'
        ...
        assert generation_found['engine'] == 'f5', \
            f"Expected engine='f5', got engine='{generation_found.get('engine')}'"
        assert generation_found['model_type'] == 'F5TTS_v1_Base', \
            f"Expected model_type='F5TTS_v1_Base', got '{generation_found.get('model_type')}'"

        print(f"✓ Generation found in history with engine='f5' and model_type='F5TTS_v1_Base'")
